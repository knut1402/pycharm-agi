...
def swap_table(a, offset = [-1], shift = [0 ,'1M' ,'3M' ,'6M' ,1], use_hist = 0):
    ...
    if isinstance(use_hist, int) == True:
        ...
        for i in np.arange(len(offset)):
            if isinstance(offset[i], int) == True:
                x1.append(ql_date_str(c.cal.advance(a.trade_date, offset[i], ql.Days)))
            else:
                x1.append(offset[i])

        t = ois_from_hist2(use_hist, x1)
    ...
    a21 = [
        pd.Series([Swap_Pricer([[t[i], 0, 2], [t[i], 0, 3]]).spread, Swap_Pricer([[t[i], 0, 2], [t[i], 0, 5]]).spread,
                   Swap_Pricer([[t[i], 0, 2], [t[i], 0, 10]]).spread, Swap_Pricer([[t[i], 0, 5], [t[i], 0, 10]]).spread,
                   Swap_Pricer([[t[i], 0, 5], [t[i], 0, 30]]).spread,
                   Swap_Pricer([[t[i], 0, 10], [t[i], 0, 30]]).spread]) for i in range(len(offset))]
    ...
    for i in range(len(offset)):
        t2 = t1.reset_index().merge(t[i].rates, how='inner', on='Tenor', suffixes=('', '_1')).set_index('index')  ### deal with non common tenors
        t1[chg[i]] = np.round(100*(t2['SwapRate'] - t2['SwapRate_1']),1)
    ...
    for i in np.arange(len(offset)):
        ...
        output_table.iloc[:, 8 + i + (2 * (n_offset - 1))] = output_table.iloc[:, 8 + i + (2 * (n_offset - 1))].round(
            decimals=1)
        output_table.iloc[:, 11 + i + (3 * (n_offset - 1))] = output_table.iloc[:, 11 + i + (3 * (n_offset - 1))].round(
            decimals=1)

    output_table.iloc[:, 4 + n_offset - 1] = output_table.iloc[:, 4 + n_offset - 1].round(decimals=3)  #### fwd rate
    output_table.iloc[:, 7 + (2 * (n_offset - 1))] = output_table.iloc[:, 7 + (2 * (n_offset - 1))].round(
        decimals=1)  #### curve rate
    output_table.iloc[:, 10 + (3 * (n_offset - 1))] = output_table.iloc[:, 10 + (3 * (n_offset - 1))].round(
        decimals=1)  #### fly rate
...
######## NEW *** curve_hmap ***  ########
def curve_hmap(a, b=0, offset=[-1], ois_flag=0):
    #    a = ['EUR_6M','PLN_6M','HUF_6M','CZK_6M']
    curve_rates = {}
    curve_rates_chg = {}
    curve_fwds = {}
    curve_crvs = {}
...
a1 = curve_hmap(['SOFR_DC'], ois_flag=1)
a1.rates
a1.curves
a1.steep
a1.roll
