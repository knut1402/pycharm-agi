...
### Get fwds dates
date_list = [x2.index[-1] + datetime.timedelta(days=x) for x in range(2500)]

### Curve builds
sofr_live =  ois_dc_build('SOFR_DC',b=0) 
sofr_fwd = [ Swap_Pricer( [[sofr_live, date_list[i].strftime('%d-%m-%Y') ,'1m']] ).rate for i in np.arange(4,len(date_list)) ]

ester_live =  ois_dc_build('ESTER_DC',b=0) 
ester_fwd = [ Swap_Pricer( [[ester_live , date_list[i].strftime('%d-%m-%Y') ,'1m']] ).rate for i in np.arange(4,len(date_list)) ]

sonia_live =  ois_dc_build('SONIA_DC',b=0)
sonia_fwd = [ Swap_Pricer( [[sonia_live , date_list[i].strftime('%d-%m-%Y') ,'1m']] ).rate for i in np.arange(4,len(date_list)) ]


### Concatenating data
usd1 = x2['S0490Z 1M BLC2 Curncy']
usd1.columns = ['sofr']
usd2 = pd.DataFrame(sofr_fwd, index = date_list[4:])
usd2.columns = ['sofr']
df = pd.concat([usd1, usd2])

eur1 = x2['S0514Z 1M BLC2 Curncy']
eur1.columns = ['ester']
eur2 = pd.DataFrame(ester_fwd, index = date_list[4:])    
eur2.columns = ['ester']
df2 = pd.concat([eur1, eur2])

gbp1 = x2['S0141Z 1M BLC2 Curncy']
gbp1.columns = ['sonia']
...
t1=list(df.index[0:428:90])+list(df.index[458:-1:122])
t2=list(x3[0:428:90])+list(x3[458:-1:122])

#### Plot hist rates + fwds
plt.plot(df)
plt.axvline(x=x2.index[-1], color = 'black')
plt.title('Base rate proxied by 1m OIS Rates', fontsize=10)
...
plt.axvline(x=x2.index[-1], color = 'black')
plt.title('Change in base rate (1m OIS) since 15-Sep-2021', fontsize=10)
plt.legend( ['USD','EUR','GBP'])
plt.xticks( t1, t2, rotation=90)
plt.grid( which='major', color='#999999', linestyle='-', alpha=0.2)
plt.tight_layout()
